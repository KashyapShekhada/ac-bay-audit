...
MAPPING_FILE = "store_mapping.xlsx"  # <-- Place your Excel file here
...
<style>
    ...
    .header-subtitle { font-size: 1rem; color: #8b949e; margin-top: 6px; }
    .header-tag {
    ...
    .placeholder {
        ...
        border: 2px dashed #30363d; color: #484f58; font-size: 1.1rem;
    }
</style>
...
if store_index is None:
    st.error("❌ store_index.json not found. Run `extract_stores.py` first.")
    st.stop()

if mapping_df is None:
    st.error(f"❌ '{MAPPING_FILE}' not found. Place your Excel mapping file in the same folder.")
    st.stop()

# Normalize store_index keys
...
# City filter — depends on region
if selected_region != "All Regions":
    city_df = mapping_df[mapping_df["Region"] == selected_region]
else:
    city_df = mapping_df
...
# Store filter — depends on region + city
...
# Only show stores that actually have images
available_sites = filtered_df[filtered_df["Site"].isin(store_index.keys())]
store_options = []
for _, row in available_sites.iterrows():
    label = f"{row['Site']} — {row['Store']} ({row['City']})"
    store_options.append((label, str(row["Site"])))

with col3:
    store_labels = ["Select a Store"] + [s[0] for s in store_options]
    selected_label = st.selectbox("🏪 Store", store_labels)
